...
public partial class Admin_Admin : System.Web.UI.Page
{
    ...
    private void ShowAdmin(string seleStr)
    {
        ...
        gridAdmin.PageSize = 15;//每页显示的记录数
        gridAdmin.DataBind();
    }
    /// <summary>
    /// "显示全部"按钮的单击事件
    ...
    protected void btnFind_Click(object sender, EventArgs e)
    {
        ...
        if (key != "")
        {
            selectStr = "Select AdminID, AdminName, AdminPassword, AdminRealName, Admin.AdminGradeID, AdminGradeName from Admin, AdminGrade where Admin.AdminGradeID = AdminGrade.AdminGradeID and (";
            selectStr += "AdminName like '%" + key + "%' ";//按管理员账号查找
            selectStr += "or AdminRealName like '%" + key + "%' ";//按真实姓名查找
            selectStr += "or Admin.AdminGradeID like '%" + key + "%' "; //按管理员级别查找
            selectStr += ")";
        }
        else
        {
